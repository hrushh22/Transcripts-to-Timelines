...
# Define function to summarize text using llama2 model
        ...
        try:
            if "llama2_summary" not in item_info:
                if "KB" in item_info:
                    ...
                    llama2_summaries = []
                    for chunk in chunks:
                        llama2_summary = summarize_with_llama2(chunk)
                        llama2_summaries.append(llama2_summary)

                    final_summary = ' '.join(llama2_summaries)

                    item_info["llama2_summary"] = final_summary

                    # Save updated JSON data
                    with open(json_output_path, 'w') as file:
                        json.dump(json_data, file, indent=4)
                        print(f"llama2 summary saved for item {item_info['ID']}")
            else:
                print(f"llama2 summary already exists for {item_info['ID']}")
